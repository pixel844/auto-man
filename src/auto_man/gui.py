...
class McpWorker(QObject):
    ...
    def initialize_backend(self):
        try:
            ...
            logger.info("GUI Backend initializing...")
            ...
            logger.success("GUI Backend ready.")
    ...
    def get_tree(self, url):
        if not self.server:
            return
        try:
            logger.info(f"Fetching tree for: {url}")
    ...
    def run_generation(self, url, repo_id=None):
        try:
            ...
            logger.info(f"Generating manual for repo ID: {self.repo_id}")
            ...
            # Clean and save
            final_content = clean_roff_content("".join(output_content))

            output_path.write_text(final_content, encoding="utf-8")

            self.progress_update.emit("Saving file...", 95)
            self.finished.emit(str(output_path))
            logger.success(f"GUI manual generation complete: {output_path}")
