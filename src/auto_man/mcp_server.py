...
class McpServer:
    ...
    def handle_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.debug(f"Handling MCP request: {method} (id: {req_id})")
        
        try:
            if method == "initialize":
                result = self.handle_initialize(req_params)
            elif method == "tools/list":
                result = self.handle_list_tools(req_params)
            elif method == "fetch_tree":
                result = self.fetch_tree(req_params)
            elif method == "add_repo":
                result = self.add_repo(req_params)
            elif method == "index_repo":
                result = self.index_repo(req_params)
            elif method == "tools/call":
                result = self.handle_call_tool(req_params)
            else:
    ...
    def fetch_tree(self, args: Dict) -> Dict:
        ...
        if is_remote:
            ...
            logger.info(f"Cloning {url} for tree preview...")
            res = subprocess.run(
        ...
        tree = []
        # Using rglob instead of os.walk for modern pathlib usage
        try:
            for item in path.rglob("*"):
                # Depth limit like before
    ...
    def generate_man(self, args: Dict) -> Dict:
        ...
        output_path.write_text(final_man_content, encoding="utf-8")
